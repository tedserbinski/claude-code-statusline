...
- **Cached git work** — one cache covers the branch, the worktree label, and the lines-changed diff, so `git` doesn't run on every statusline tick. It refreshes on a 5-second TTL **or the moment the checkout moves**: the cache remembers the content of `.git/HEAD` and the index's mtime, so a `git checkout`, `add`, `commit`, or `reset` shows on the very next tick instead of up to 5 s later. A cache hit costs zero subprocesses (no `stat`, no `git`); a refresh is five `git` calls regardless of how many mainline or parent refs exist.
...
| **Lines changed** | `+35/-31` | green / red | Real `git diff` of the current branch/worktree against its fork point — every tracked line **added/removed since the branch diverged from the mainline**, counting committed, staged, and unstaged edits together. The fork point is the **newest** merge-base against any of the branch's candidate parents: the mainline refs (`main`, `master`, `origin/HEAD`, `origin/main`, `origin/master`) **and, for stacked branches, the recorded parent** — read from whichever tool wrote it: `gh` (`branch.<b>.gh-merge-base`), git-town (`git-town-branch.<b>.parent`), Graphite (`refs/branch-metadata/<b>`), or plain git's local upstream (`git branch --set-upstream-to=parent`). So a feature branch shows all its work, a child branch shows only its own lines rather than its parent's, merging a newer `origin/main` into a branch does not count the upstream lines even when local `main` is stale, and `main` itself shows just uncommitted changes. Untracked files are excluded, and the whole segment is hidden when the tree is clean. Computed inside the git cache, so no extra cost per tick. |
...
A test suite covering 63 checks (full payloads, boundary values, missing fields, model-name shortening, braille bars, absolute reset times, percentage clamping, cross-session rate sync, boosted/reset limits, the usage endpoint (fixture-fed, non-blocking, backoff), per-account rate isolation, the weekly window, worktree labelling, lines-since-fork with a stale local `main`, instant refresh on branch switch, stacked branches under every parent convention, rapid redraws, performance) is included:

```sh
bash ~/claude-statusline/test-statusline.sh
```

Expected output ends with `All 63 tests passed ✓`. If any test fails, the output line count or stderr leakage is almost always the cause — see the "Troubleshooting" section below.
...
- **Git cache TTL** — defaults to 5 seconds (search for `now_ts - c_at >= 5`). Raise it if your repo is huge and git calls are slow; a branch switch or commit still refreshes immediately because the cache is also keyed on `.git/HEAD` and the index mtime.
