...
A test suite covering 52 checks (full payloads, boundary values, missing fields, model-name shortening, braille bars, absolute reset times, percentage clamping, cross-session rate sync, boosted/reset limits, per-account rate isolation, the weekly window, worktree labelling, rapid redraws, performance) is included:
